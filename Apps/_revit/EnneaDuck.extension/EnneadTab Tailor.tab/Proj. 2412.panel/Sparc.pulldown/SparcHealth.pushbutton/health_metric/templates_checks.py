...
def check_templates_filters(doc):
    ...
    try:
        ...
        # Check if document is workshared (for safe WorksharingUtils access)
        is_workshared = False
        try:
            is_workshared = doc.IsWorkshared
        except:
            pass  # If check fails, assume not workshared
        ...
        for template in all_templates:
            try:
                ...
                # Get creator and last editor from WorksharingUtils (only if workshared)
                # CRITICAL: WorksharingUtils.GetWorksharingTooltipInfo crashes on cloud models
                if is_workshared:
                    try:
        ...
        # Add creator and last editor statistics (only if workshared)
        templates_data["view_template_creators"] = _analyze_template_creators(doc, all_templates, is_workshared)
...
def _analyze_template_creators(doc, templates, is_workshared):
    """Analyze view template creators and last editors"""
    try:
        creator_data = {}
        last_editor_data = {}
        
        # CRITICAL: Skip WorksharingUtils on non-workshared or cloud models to prevent crash
        if not is_workshared:
            return {
                "creators": {},
                "last_editors": {},
                "note": "Worksharing info not available (cloud model or non-workshared)"
            }
